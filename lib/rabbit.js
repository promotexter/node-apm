...
function RabbitDriver(opts) {
  ...
  this.channel 	= opts.channel || 'node-apm';
  this.host 	= opts.host || 'amqp://localhost';
  this.expiration = opts.expiration || (1000); // 1 second default message expire time
...
RabbitDriver.prototype.init = function(callback) {
	...
	context.on('ready', function() {
	  ...
	  publisher.connect(channel, function() {
	      ...
	      console.log('pub connected',host, channel, expiration);
